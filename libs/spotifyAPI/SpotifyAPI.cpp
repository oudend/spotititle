...
void SpotifyAPI::ConfigureAPIConnection()
{
    // Copy from SpotifyAPI's TestEndpoints.cpp.
    std::string refreshToken = GetStringFromFile("spotify_config_refresh_token.txt");
    std::string clientId = GetStringFromFile("spotify_config_client_id.txt");
    std::string clientSecret = GetStringFromFile("spotify_config_client_secret.txt");
...
{
    SpotifyPOST("/v1/me/player/next", options, authToken);
}

void SpotifyAPI::AddItemToPlaybackQueue(std::string trackId, options_t options)
{
    options["uri"] = trackId;
    SpotifyPOST("/v1/me/player/queue", options, authToken);
}   
